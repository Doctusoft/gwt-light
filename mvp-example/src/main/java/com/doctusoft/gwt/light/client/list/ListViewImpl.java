	...
	protected void onTemplateLoaded() {
		new LightList<TestEntityDTO>("#entityList") {
			...
			protected void renderItem(JQuery row, TestEntityDTO item, int number) {
				...
				new LightButton(row.find(".editButton")).click(presenterMethod(ListActivity.__editEntity, item));
				new LightButton(row.find(".removeButton")).click(presenterMethod(ListActivity.__removeEntity, item));
			}
